...
func VerifyToken(csp bccsp.BCCSP, token string, method, uri string, body []byte, compMode1_3 bool) (*x509.Certificate, error) {
	...
	log.Debugf("解析token后得到的Sig = %s", b64Sig)
	log.Debugf("解析token后得到的Cert= %s", b64Cert)
	if err != nil {
		return nil, err
	}
	...
	tmp1, err1 := pk2.Bytes()
	if err1 != nil {
		return nil, errors.New("pk2.Bytes() error!!!")
	}
	log.Infof("pk2公钥base64：%s", B64Encode(tmp1))
	...
	if compMode1_3 && !valid {
		...
		log.Debugf("生成摘要前的字符串 = %s", sigString)
		digest, digestError := csp.Hash([]byte(sigString), &bccsp.SHAOpts{})
		log.Debugf("=============bodyandcert一样的，看fabric-ca生成的摘要：%s", B64Encode(digest))
		if digestError != nil {
			return nil, errors.WithMessage(digestError, "Message digest failed")
		}
		valid, validErr = csp.Verify(pk2, sig, digest, nil)
